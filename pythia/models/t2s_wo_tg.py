...
@registry.register_model("t2s_wo_tg")
class T2S(BaseModel):
...
class Grounding_Module(nn.Module):
    ...
    def forward(self, sample_list, fwd_results):
        ...
        # w/o TG
        ground_ocr_box, ground_ocr_mask, neg_ocr_mask =   self.ocr_grounding_indicator(
            global_q_feat, ocr_feat, ocr_box, ocr_mask, new_ocr_mask, self.frame_topk, self.frame_topk*self.ocr_topk, self.frame_num, self.frame_ocr_num
            )
        ground_ocr_mask = ground_ocr_mask * ocr_mask
        neg_ocr_mask = neg_ocr_mask * ocr_mask
        # print(torch.sum(ground_ocr_mask, dim=1).tolist())


        # w/o TG: obtain pos and neg frame mask
        reshape_ground_ocr_mask = ground_ocr_mask.view(B, self.frame_num, -1)
        pos_one = reshape_ground_ocr_mask.any(dim=2)  # Shape: [batchsize, frame_num]
        pos_indices = [torch.where(pos_one[i])[0] for i in range(pos_one.shape[0])]
        padded_pos_indices = [torch.cat([idx, torch.full((5 - len(idx),), -1, dtype=torch.long, device=ground_ocr_mask.device)])[:5] if len(idx) < 5 else idx[:5] for idx in pos_indices]
        ground_frame_idx = torch.stack(padded_pos_indices) # 
        ground_frame = ground_frame_idx
        # obtain pos frame mask
        batchsize = ground_frame_idx.shape[0]
        ground_frame_mask = torch.zeros((batchsize, self.frame_num), dtype=torch.long, device=ground_frame_idx.device)  # 
        row_indices = torch.arange(batchsize).unsqueeze(1)  # Shape: [batchsize, 1]
        ground_frame_mask[row_indices, ground_frame_idx] = 1
        

        reshape_neg_ocr_mask = neg_ocr_mask.view(B, self.frame_num, -1)
        neg_one = reshape_neg_ocr_mask.any(dim=2)  # Shape: [batchsize, frame_num]
        neg_indices = [torch.where(neg_one[i])[0] for i in range(neg_one.shape[0])]
        padded_neg_indices = [torch.cat([idx, torch.full((5 - len(idx),), -1, dtype=torch.long, device=ground_ocr_mask.device)])[:5] if len(idx) < 5 else idx[:5] for idx in neg_indices]
        neg_frame_idx = torch.stack(padded_neg_indices)
        # obtain neg frame mask
        batchsize = neg_frame_idx.shape[0]
        neg_frame_mask = torch.zeros((batchsize, self.frame_num), dtype=torch.long, device=ground_frame_idx.device)  # 
        row_indices = torch.arange(batchsize).unsqueeze(1)  # Shape: [batchsize, 1]
        neg_frame_mask[row_indices, neg_frame_idx] = 1
