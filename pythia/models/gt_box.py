...
@registry.register_model("gt_box")
class GTBOX(BaseModel):
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results):
        ...
        ocr_temporal_id = sample_list.ocr_temporal_id
        ocr_temporal_id = self.temporal_position_embeddings(ocr_temporal_id)
        ocr_track_id = sample_list.ocr_track_id
        ocr_track_id = self.track_position_embeddings(ocr_track_id)
        ocr_bbox = sample_list.ocr_bbox_list
        ocr_mask = sample_list.ocr_mask_embedding
...
class Grounding_Module(nn.Module):
    ...
    def forward(self, sample_list, fwd_results):
        ...
        # start: gt box as input----------------------------------------

        fwd_results['ground_frame'] = sample_list.frame_list
        fwd_results['ground_bbox'] = sample_list.ocr_bbox_list
        fwd_results['frame_topk'] = torch.tensor(64,device=frame_feat.device)
        fwd_results['ocr_topk'] = torch.tensor(15,device=frame_feat.device)

        fwd_results['pos_obj_mmt_in'] = frame_feat 
        fwd_results['pos_obj_mask'] = sample_list.frame_mask_embedding
        fwd_results['pos_ocr_mmt_in'] = ocr_feat
        fwd_results['pos_ocr_mask'] =  sample_list.ocr_mask_embedding
...
class MMT(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        # dec_ocr_mask = ocr_mask.unsqueeze(-1).expand(ocr_emb.size(0), -1, ocr_emb.size(-1))
        # ocr_emb = ocr_emb * dec_ocr_mask
...
class PrevPredEmbeddings(nn.Module):
    ...
    def forward(self, ans_emb, ocr_emb, prev_inds):
        ...
        ans_ocr_emb_cat = torch.cat([ans_emb, ocr_emb], dim=1)  # [bs, 5964, 768]
