...
class M4C(BaseModel):
    ...
    def _forward_obj_encoding(self, sample_list, fwd_results):
        ...
        video_frcn = sample_list.mid_img_feat # sample_list.video_feat
        ...
        video_mmt_in = (
            self.obj_feat_layer_norm(
                self.linear_obj_feat_to_mmt_in(video_frcn)
                )
        )
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results):
        ...
        ocr_feat = torch.cat(
            [ocr_fasttext, ocr_phoc], # no temporal info
            dim=-1
        )
...
class PostHoc_Attention(nn.Module):
    ...
    def forward(self, sample_list, fwd_results):
        ...
        ocr_mask = fwd_results['ocr_mask']
        frame_features = fwd_results['obj_mmt_in']
        frame_mask = fwd_results['obj_mask']
        q_feat = fwd_results['txt_emb']
        q_mask = fwd_results['txt_mask']

        B, _, D = frame_features.size()[:3]
        num_frame = self.frame_num

        '''1. obtain the middle frame'''
        middel_frame_id = sample_list.middel_frame_id # B, 1
        middel_frame_idx = sample_list.middel_frame_idx
        f_topk_ids = torch.zeros((B, num_frame),device=frame_features.device) # B, 64
        new_frame_mask = f_topk_ids.scatter_(1, middel_frame_idx -1, 1)

        '''2. obtain all ocr tokens in the middle frame'''
        # obtain top-k grounded ocr idx in each frame
        tensor1 = middel_frame_id
        ...
        middle_frame_mask = new_frame_mask * frame_mask
        # middle ocr with padding
        new_ocr_mask.scatter_(1, new_ocr_idx, 1)
        # # middle ocr without padding
        middle_ocr_mask = new_ocr_mask * ocr_mask

        
        '''3. post-hoc attention analysis'''
        ...
        reshape_ocr_score = ocr_score.view(B, num_frame, self.ocr_frame_num)  # [bs, frame_num, ocr_frame_num]
        # ground top-k ocr in the middle frame
        _, sorted_indices = torch.sort(reshape_ocr_score, descending=True, dim=-1)  # [bs, frame_num, ocr_frame_num]
        topk_indices = sorted_indices[:,:,:self.ocr_topk]  # [bs, frame_num, o_topk]
        topk_mask = torch.zeros_like(reshape_ocr_score, device=ocr_feat.device)
        topk_mask.scatter_(2, topk_indices, 1) # [bs, frame_num, ocr_frame_num]
        assert torch.sum(topk_mask).item() == B * self.ocr_topk * self.frame_num
        topk_mask = topk_mask.view(B, -1)  

        ground_topk_mask = (topk_mask * new_ocr_mask)
        ground_topk_box_mask = ground_topk_mask.unsqueeze(-1).expand(B, -1, 4) 
        ground_ocr_box = torch.masked_select(ocr_box, ground_topk_box_mask.bool()).view(B, -1, 4)  # [bs, ocr_topk, 4]
        ground_ocr_mask = torch.masked_select(ocr_mask, ground_topk_mask.bool()).view(B, -1)
        ground_ocr_box = ground_ocr_box * (ground_ocr_mask.unsqueeze(-1).expand(B, -1, 4))


        # grounding evaluation
        fwd_results['ground_frame'] = middel_frame_id
        fwd_results['ground_bbox'] = ground_ocr_box
        fwd_results['frame_topk'] = torch.tensor(self.frame_topk,device=middel_frame_id.device)
        fwd_results['ocr_topk'] = torch.tensor(self.ocr_topk,device=ground_ocr_box.device)

        # answer prediction
        fwd_results['obj_mmt_in'] = frame_features 
        fwd_results['obj_mask'] = torch.ones((B, 1),dtype=torch.float32, device=frame_features.device)
        fwd_results['ocr_mmt_in'] = ocr_feat
        fwd_results['ocr_mask'] = middle_ocr_mask
