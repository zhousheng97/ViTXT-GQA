...
@registry.register_model("t2s_wo_sg")
class T2S(BaseModel):
...
class Grounding_Module(nn.Module):
    ...
    def forward(self, sample_list, fwd_results):
        ...
        # ablation: w/o SG
        ground_ocr_mask = new_ocr_mask  # all tokens of the grounded frames
        neg_ocr_mask = torch.ones_like(ground_ocr_mask) - ground_ocr_mask  # all tokens of the non-grounded frames
        ocr_topk_mask = new_ocr_mask.unsqueeze(-1).expand(B, -1, 4)
        ground_ocr_box = torch.masked_select(ocr_box, ocr_topk_mask.bool()).view(B, -1, 4)  # [bs, frame_num*ocr_frame_num, 4]


        # grounding evaluation
        fwd_results['ground_frame'] = ground_frame
        fwd_results['ground_bbox'] = ground_ocr_box
        fwd_results['frame_topk'] = torch.tensor(self.frame_topk,device=ground_frame.device)
