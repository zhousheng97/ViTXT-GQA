...
class BaseTrainer:
    ...
    def _update_meter(self, report, meter=None, eval_mode=False):
        ...
        with torch.no_grad():
            ...
            meter_update_dict = {total_loss_key: reduced_loss}
            meter_update_dict.update(reduced_loss_dict)
            meter_update_dict.update(reduced_metrics_dict)
            meter.update(meter_update_dict)

            # print all loss
            # for idx, loss_list in enumerate(loss_dict):
            #     loss_name = loss_list.split('/')[-1]
            #     loss_key = report.dataset_type + '/'+loss_name
            #     loss_value = loss_dict[loss_list].mean()
            #     registry.register(loss_key, loss_value)

            #     meter_update_dict = {loss_key: loss_value}
            #     meter.update(meter_update_dict)
