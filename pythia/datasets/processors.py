...
class BertTokenizerProcessor(BaseProcessor):
    ...
    def __init__(self, config, *args, **kwargs):
        self.max_length = config.max_length
        self.bert_tokenizer = BertTokenizer.from_pretrained('/data/zsheng/huggingface/bert-base-uncased/') # 
...
class M4CAnswerProcessor(BaseProcessor):
    ...
    def match_answer_to_vocab_ocr_seq(
        ...
        if len(answer_word_matches) == 0:
            return []
        idx_seq_list = [()]
        for matched_inds in answer_word_matches:   # sort answer_word_matches from front to back
            idx_seq_list = [
                seq + (idx,)
    ...
    def __call__(self, item):
        ...
        # from pythia.models.transtr import Model
        # Model().forward()
        # from pythia.modules.losses import M4CDecodingBCEWithMaskLoss
        # loss = M4CDecodingBCEWithMaskLoss().process_answer_info()
