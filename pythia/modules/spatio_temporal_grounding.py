...
class Temporal_Grounding_Indicator(nn.Module):
    ...
    def forward(self, q_global, v_local, attn_mask, sample_list, topk):
        ...
        return ground_frame, pos_topk_mask, neg_topk_mask #, ground_frame_feat
...
class Spatial_Grounding_Indicator(nn.Module):
    ...
    def forward(self, q_global, v_local, v_box, v_mask, attn_mask, f_topk, o_topk, frame_num, o_frame_num):
        ...
        # print("pos_topk_mask:", torch.sum(pos_topk_mask).item(), "pos_topk_mask.shape:", pos_topk_mask.shape)
        ...
        # assert torch.sum(neg_topk_mask).item() == B * o_topk * f_topk
        
        pos_topk_mask = pos_topk_mask * attn_mask 
        pos_topk_box_mask = pos_topk_mask.unsqueeze(-1).expand(B, -1, 4)
        ground_ocr_id = torch.masked_select(v_box, pos_topk_box_mask.bool()).view(B, -1, 4)  # [bs, frame_num*ocr_frame_num, 4]

        return ground_ocr_id, pos_topk_mask, neg_topk_mask 