...
@registry.register_metric("temporal_acc_R@k")
class TempGroundAccuracy(BaseMetric):
    '''
    the grounded fame is one of the frames of the labeled temporal span is  correct
    '''
    def __init__(self):
        super().__init__("temporal_acc_R@k")
        import pythia.utils.m4c_evaluators as evaluators
        self.evaluator = evaluators.TempGroundAccuracyEvaluator()
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        t_accuracy = self.evaluator.eval_pred_list(predictions)
        t_accuracy = torch.tensor(t_accuracy).to(sample_list.frame_num.device)  # .cuda()
...
class BoxGroundAccuracy(BaseMetric):
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        pred_scores = []
        _, box_accuracy = self.evaluator.eval_pred_list(pred_scores, predictions, threshold=0.1)
...
@registry.register_metric("box_gqa_R@k_IOU@0.5")
class BoxGroundAccuracy(BaseMetric):
    '''
    the IoU of the grounded bbox and the labeled bbox are greater than 0.5 indicates that the location is correct
    '''
    def __init__(self):
        super().__init__("box_gqa_R@k_IOU@0.5")
        import pythia.utils.m4c_evaluators as evaluators
...
@registry.register_metric("box_gqa_R@k_IOU@0.3")
class BoxGroundAccuracy(BaseMetric):
    '''
    the IoU of the grounded bbox and the labeled bbox are greater than 0.5 indicates that the location is correct
    '''
    def __init__(self):
        super().__init__("box_gqa_R@k_IOU@0.3")
        import pythia.utils.m4c_evaluators as evaluators
        self.box_evaluator = evaluators.BoxGroundAccuracyEvaluator()
        self.qa_evaluator = evaluators.TextVQAAccuracyEvaluator()
...
@registry.register_metric("box_gqa_R@k_IOU@0.1")
class BoxGroundAccuracy(BaseMetric):
    '''
    the IoU of the grounded bbox and the labeled bbox are greater than 0.5 indicates that the location is correct
    '''
    def __init__(self):
        super().__init__("box_gqa_R@k_IOU@0.1")
        import pythia.utils.m4c_evaluators as evaluators
        self.box_evaluator = evaluators.BoxGroundAccuracyEvaluator()
        self.qa_evaluator = evaluators.TextVQAAccuracyEvaluator()
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        box_pred_scores = []
        box_pred_scores, _ = self.box_evaluator.eval_pred_list(box_pred_scores, box_predictions, threshold=0.1)
