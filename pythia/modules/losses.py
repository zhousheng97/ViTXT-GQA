...
@registry.register_loss("neg_kl_loss")
class NEGBCEWithMaskLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.])
    ...
    def forward(self, sample_list, model_output):
        pred_score = model_output["neg_scores"]
        res = F.log_softmax(pred_score, dim=-1)
        kl_loss = self.kl_loss(res, pred_score.new_ones(pred_score.size())).view(1)
    
        return kl_loss


@registry.register_loss("contrastive_loss")
class CONTRASTIVEWithMaskLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.])
    ...
    def forward(self, sample_list, model_output):
        loss_mask = sample_list["train_loss_mask"]
        targets = sample_list["targets"]
        count = torch.max(torch.sum(loss_mask), self.one.to(loss_mask.device))

        # postive loss
        pos_scores = model_output["pos_scores"]
        assert pos_scores.dim() == 3 and loss_mask.dim() == 2
        pos_losses = F.binary_cross_entropy_with_logits(
            pos_scores, targets, reduction="none"
        )
        pos_losses *= loss_mask.unsqueeze(-1)
        pos_loss = torch.sum(pos_losses) / count

        # negative loss
        neg_scores = model_output["neg_scores"]
        assert neg_scores.dim() == 3 and loss_mask.dim() == 2
        neg_losses = F.binary_cross_entropy_with_logits(
            neg_scores, targets, reduction="none"
        )
        neg_losses *= loss_mask.unsqueeze(-1)
        neg_loss = torch.sum(neg_losses) / count

        tmp_0 = torch.zeros_like(neg_losses).cuda()
        loss = torch.max(pos_loss-neg_loss+300, tmp_0).mean().view(1)
...
class InfoNCE(nn.Module):
    ...
    def forward(self, sample_list, model_output, temperature=0.1, reduction='mean', negative_mode='paired'):
        ...
        # positive_logit = torch.sum(query * positive_key, dim=1, keepdim=True) # [bs, 1]


        negative_key = negative_key.view(batch_size, -1)
        negative_logit  = F.cosine_similarity(query, negative_key, dim=1).unsqueeze(1)
        # negative_logit = torch.sum(query * negative_key, dim=1, keepdim=True) # [bs, 1]


        # First index in last dimension are the positive samples
        logits = torch.cat([positive_logit, negative_logit], dim=1) # [bs, 2]
        labels = torch.zeros(logits.size(0), dtype=torch.long, device=logits.device)  # [24]  标签的下标为0代表第一个元素是positive

        loss = F.cross_entropy(logits / temperature, labels, reduction=reduction) # [48, 5964], [48]
...
class ANLS_REWARD(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.])
        fixed_ans_path ='/data/zsheng/Data_T5_ViteVQA/data/m4vitevqa/vocabulary/fixed_vocab_top5k_t1s2.txt'
    ...
    def forward(self, sample_list, model_output):
        ...
        # gt_answers = sample_list['gt_answers_enc']
